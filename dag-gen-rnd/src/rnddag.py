...
class DAG:
    ...
    def gen_rnd(self, parallelism=8, layer_num_min=5, layer_num_max=12, connect_prob=0.5):
        ...
        # add the root node with task_type as "CPU"
        n = 1
        G.add_node(n, rank=0, task_type="CPU")
        nodes.append([n])
        nodes_parent.append(n)
        n = n + 1
        ...
        for k in range(layer_num_this):
            ...
            for _ in range(m):
                ...
                nodes_orphan.append(n)
                # 给这个节点随机分配一个 CPU/GPU
                task_type = "CPU" if random() < 0.5 else "GPU"

                # 在添加节点时附带上 task_type
                G.add_node(n, rank=k+1, task_type=task_type)
                n = n + 1
        ...
        # Dealing with the final layer
        # Add the final node explicitly with task_type "CPU"
        G.add_node(n, rank=layer_num_this+1, task_type="CPU")
        # connect everything together to a final node
        for i in nodes_parent:
            G.add_edge(i, n)
